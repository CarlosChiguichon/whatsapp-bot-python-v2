...
class OpenAIService:
    ...
    Implementa cache y manejo de errores mejorado.
    """
    ...
    def clear_thread(self, user_id):
        """
        Elimina el thread asociado a un usuario.
        Útil cuando se cierra una sesión o se reinicia una conversación.
        ...
        if user_id in self.thread_cache:
            try:
                thread_id = self.thread_cache[user_id]
                # Actualmente la API no soporta eliminar threads, pero podemos eliminar de la caché
                del self.thread_cache[user_id]
...
def get_openai_service():
    """
    Crea y retorna una instancia del servicio de OpenAI.
    """
    try:
        api_key = current_app.config.get("OPENAI_API_KEY")
        assistant_id = current_app.config.get("ASSISTANT_ID")
        return OpenAIService(api_key=api_key, assistant_id=assistant_id)
    except RuntimeError:
